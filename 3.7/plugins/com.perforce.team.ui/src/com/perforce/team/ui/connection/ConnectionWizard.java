 ...
 */
public class ConnectionWizard extends AbstractConnectionWizard implements IConnectionWizard, INewWizard {
	...
	//
	// final List<IP4Container> retrievedFolders = new
	// ArrayList<IP4Container>();
	...
	private ConnectionParameters createParameters() {
		...
		params.setIgnoreSSLValidation(isIgnoreSSLValidation());
		...
		try {
			getContainer().run(true, false, new IRunnableWithProgress() {
				...
				public void run(IProgressMonitor monitor) throws InvocationTargetException, InterruptedException {
					...
					monitor.beginTask(
							MessageFormat.format(Messages.ConnectionWizard_AddingConnection, params.getPort()), 4);
	...
	private void checkSwarm(Shell shell) {
		ReviewManager<IP4PendingChangelist> swarmReviewManager = PendingReviewManager
				.getInstance(super.getConnection());
		...
		if (!result.isSuccessful()) {
			...
			if (!result.isReportInSilence()) {
				MessageDialog.openInformation(shell, "P4 Code Review Connection", result.getMessage());
			}
		}
	}
	...
	private boolean createClient(ConnectionParameters params, String stream, IProgressMonitor monitor) {
		// Create client if needed
		monitor.subTask(NLS.bind(Messages.ConnectionWizard_CreatingClient, params.getClient()));
		ClientBuilder builder = new ClientBuilder(params, clientPage.getLocation(), stream);
		IErrorHandler handler = new ErrorHandler() {

			@Override
			public boolean shouldRetry(IP4Connection connection, final P4JavaException exception) {
				boolean retry = false;
		...
		if (!builder.build(handler)) {
			PerforceUIPlugin.syncExec(new Runnable() {
				...
				public void run() {
					...
					P4ConnectionManager.getManager().openError(getShell(), Messages.ConnectionWizard_ClientExistsTitle,
							NLS.bind(Messages.ConnectionWizard_ClientExistsMessage, getClient()));
				}
			});
			return false;
		}
	...
	public boolean isIgnoreSSLValidation() {
		return serverPage.isIgnoreSSLValidation();
	}
}
