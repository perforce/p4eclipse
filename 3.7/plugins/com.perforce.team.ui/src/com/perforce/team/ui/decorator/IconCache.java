...
public class IconCache {
     ...
     * @param base
     * @param project
     * @return - decorated icon if project is a P4 shared project
     */
    ...
    public Image getImage(Image base, IP4PendingChangelist list) {
        if (list != null && (list.isShelved() || list.hasReview())) {
            Image decorated = base;
            List<ImageDescriptor> idList = new ArrayList<ImageDescriptor>();
            idList.add(getShelvedDecoration());
            if(list.hasReview()){
            	idList.add(getChangeListInReviewDecoration());
            }
            ...
            iconOlArray[0] = IPerforceUIConstants.ICON_TOP_LEFT;
            if(list.hasReview()){
            	iconOlArray[1] = IPerforceUIConstants.ICON_BOTTOM_RIGHT;
            }
            OverlayIcon icon = new OverlayIcon(base, descriptors,iconOlArray);
    ...
    public Image getImage(Image base, IP4SubmittedChangelist list) {
        if (list != null && list.hasReview()) {
            ...
            if(list.hasReview() && iconOlArray.length > 0){
            	iconOlArray[0] = IPerforceUIConstants.ICON_BOTTOM_RIGHT;
            }
            OverlayIcon icon = new OverlayIcon(base, descriptors,iconOlArray);
            Image image = cache.get(icon);
            if (image == null) {
	...
	private ImageDescriptor getChangeListInReviewDecoration() {
		return createAndGetImageDesc(IPerforceUIConstants.IMG_CHANGELIST_IN_REVIEW);
	}
