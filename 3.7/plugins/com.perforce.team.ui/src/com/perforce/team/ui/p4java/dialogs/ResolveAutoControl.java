        ...
        //for (int i = 0; i < table.getItemCount(); i++) {
            ...
            //if (op.file != item.file) {
               ...
               //}
                ...
                op.options.setChangelistId(changeId);
           // }
        ...
        for (ResolveOperation op : operations) {
            ...
            IP4Runnable runnable = new P4Runnable() {
                ...
                public void run(IProgressMonitor monitor) {
                    // the real resolve
                    final int numIntegSpecsBefore = 0;//file.getIntegrationSpecs() != null ? file.getIntegrationSpecs().length : 0;
                    collection.resolveChangelist(options, conn);
                    // followed by a preview resolve to refresh
                    //file.refresh();
                    collection.resolve(new ResolveFilesAutoOptions().setShowActionsOnly(true)
                            .setShowBase(true));
                    final int numIntegSpecsAfter = 0;//file.getIntegrationSpecs() != null ? file.getIntegrationSpecs().length : 0;
                    UIJob job = new UIJob(
