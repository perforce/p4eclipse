...
public class P4Depot extends P4Container {
	...
	/**
	 * Creates a new p4 depot object from a P4J depot metadata object and a p4 connection object.
	 * 
	 * @param depotData
	 * @param connection
	 */
			...
			if (client != null) {
				...
				if (view != null) {
					...
					for (IClientViewMapping mapping : view) {
						if (isCaseSensitive()) {
							if (startsWith(mapping.getDepotSpec(), depotPath)) {
								local = true;
								break;
							}
						} else {
							if (startsWithIgnoreCase(mapping.getDepotSpec(), depotPath)) {
								local = true;
								break;
							}
						}
