...
public class IntegrateDialogTest extends ProjectBasedTestCase {
    ...
    public void testDialog() {
        ...
        try {
            ...
            dialog.open();
            assertEquals(IChangelist.DEFAULT, dialog.getPendingChangelistId());
            assertNotNull(dialog.getSourcePath());
            assertNotNull(dialog.getTargetPath());
            assertNull(dialog.getEndRevisionOption());
            assertNull(dialog.getStartRevisionOption());
            assertNull(dialog.getBranchName());
            assertTrue(dialog.isFileIntegration());
            dialog.setBaselessMerge(true);
            dialog.setDisregardHistory(true);
            dialog.setDoNotCopy(true);
            dialog.setDoNotGetLatestRevision(true);
            ...
            P4FileIntegration integ = dialog.getCurrentFileIntegrationInfo();
            assertNotNull(integ);
    ...
    public void testPreview() {
        ...
        try {
            ...
            IP4Resource[] previewed = createConnection().integratePreview(options, "", 0, "", true, new P4IntegrationOptions3(), false);
