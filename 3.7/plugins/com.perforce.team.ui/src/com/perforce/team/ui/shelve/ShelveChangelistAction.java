...
public class ShelveChangelistAction extends P4Action {
	...
	private void shelveDefault(final IP4PendingChangelist list) {
		IP4Runnable runnable = new P4Runnable() {
			...
			public void run(IProgressMonitor monitor) {
				...
				action.selectionChanged(null, new StructuredSelection(list.members()));
	 ...
	 * @param selected
	 */
	public void updateShelveNumbered(final IP4PendingChangelist list, final IP4Resource[] selected) {
	...
	public void updateShelveNumbered(final IP4PendingChangelist list, final IP4Resource[] selected,
			final Option option) {
		final IP4ShelvedChangelist shelvedList = new P4ShelvedChangelist(list.getConnection(), list.getChangelist(),
				true);
		...
		PerforceUIPlugin.syncExec(new Runnable() {
			public void run() {
				UpdateShelveDialog dialog = new UpdateShelveDialog(P4UIUtils.getDialogShell(), list, selected,
						shelvedFiles, pendingFiles, option);
					...
					if (changes[0] != null && changes[0].length > 0) {
						...
						if (shelveUpdates.length > 0) {
							...
							if (shelveUpdates[0].option == Option.REPLACE) {
								...
								collectionSelection = createCollection(pendingFiles);
							} else {
								...
								for (ShelveChange change : shelveUpdates) {
									...
									} else if (change.option == Option.ADD || change.option == Option.UPDATE) {
								...
								collectionSelection = createCollection(updates.toArray(new IP4Resource[0]));
							}
			
							// revert/wipe files if option set
