...
public class CoreUtilsTest extends ProjectBasedTestCase {
    ...
    public void testPathFormatting() {
        ...
        String windowspath = "C:\\dir1\\dir2\\t@e#s*t%5.txt";
        String depotString = "//depot/test1/test2.txt";
        String[] stringArray = new String[] { at, hash, asterix, normal, percent, windowspath, depotString };
        String[] postFormatChars = new String[] {"%40", "%23", "%25", "%2A"};

        String[] output = PerforceConnectionFactory
                .formatFilenames(stringArray);
        assertNotNull(output);
        assertEquals(7, output.length);

        assertNotNull(output[0]);
        assertFalse(output[0].contains("@"));
        assertTrue(output[0].contains("%40"));

        assertNotNull(output[1]);
        assertFalse(hash.equals(output[1]));
        assertFalse(output[1].contains("#"));
        assertTrue(output[1].contains("%23"));

        assertNotNull(output[2]);
        assertFalse(asterix.equals(output[2]));
        assertFalse(output[2].contains("*"));
        assertTrue(output[2].contains("%2A"));
        ...
        assertTrue(output[4].contains("%25"));
        
        assertNotNull(output[5]);
        assertFalse(percent.equals(output[5]));
        for (String s : postFormatChars) {
            assertTrue(output[5].contains(s));
        }
        
        assertNotNull(output[6]);
        assertTrue(output[6].equals(depotString));
    }
