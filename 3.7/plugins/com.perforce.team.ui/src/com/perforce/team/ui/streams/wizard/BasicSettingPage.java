...
//    private Button createWorkspaceBtn;
    ...
    public void createControl(Composite parent) {
        ...
        depotName = StreamUtil.getParentDepot(sum);
        if(depotName != null){
        	 nameText.setText(depotName);
        }
        String path = sum.getParent();
        if(path != null){
        	segments = path.substring(depotName.length()).split("/");
        }
    ...
    protected void addListeners() {
        ...
        nameText.addKeyListener(new KeyListener(){
			...
			public void keyPressed(KeyEvent e) {
				if(e.keyCode == SWT.BS || e.keyCode == SWT.DEL){
					if(nameText.getCaretPosition() <= depotName.length()){
						e.doit = false;
						return;
					}
				} else {
					String namePart = nameText.getText().substring(depotName.length());
					boolean isDepthSame = segments.length == namePart.split("/").length;
					if(e.keyCode == 47 && nameText.getText().endsWith("/")){
						e.doit = false;
						return;
					}
        ...
        nameText.addModifyListener(new ModifyListener() {
            public void modifyText(ModifyEvent e) {
                getStream().setName(nameText.getText().substring(depotName.length()));
                getStream().setStream(nameText.getText());
                if(!getWizard().isEditMode()){
                    String depot=StreamUtil.getDepot(getStream());
                    ...
                    if(!StringUtils.isEmpty(depot) && !getWizard().rootFolderChanged){
                        StreamUtil.updateStreamName(getStream(), getStream().getName(), depot);
                    }
                }
            }
        });
