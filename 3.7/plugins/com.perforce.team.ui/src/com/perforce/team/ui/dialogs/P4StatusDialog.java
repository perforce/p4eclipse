 ...
 */
public abstract class P4StatusDialog extends StatusDialog implements IErrorDisplay {
	...
	protected IDialogSettings getDialogBoundsSettings() {
		...
		if (perforceUIPlugin != null) {
			IDialogSettings settings = perforceUIPlugin.getDialogSettings();
			if (settings != null) {
	...
	public void setMessage(int severity, String message) {
		IStatus status = null;
		if (message != null) {
			status = new Status(severity, PerforceUIPlugin.ID, Status.OK, message, null);
		} else {
			...
			public void modifyText(ModifyEvent e) {
				if (text.getEnabled() == true) {
					...
					if (error != null) {
						...
						textDecorator.show();
						setErrorMessage(MessageFormat.format("Illegal {0} name.", formTypeName));
					} else {
						textDecorator.hide();
