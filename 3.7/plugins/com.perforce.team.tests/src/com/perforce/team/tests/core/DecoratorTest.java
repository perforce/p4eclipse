...
public class DecoratorTest extends ProjectBasedTestCase {
    ...
    public void setUp() throws Exception {
        ...
        for (int i = 0; i < 5; i++) {
            ...
            addFileWithContent(client, project.getFile("plugin.xml"), "version: " + i);
            addFile(client, project.getFile("deletefile.xml"));
        }
...
//        view.getViewer().expandToLevel(2);
    ...
    public void testDeleteImage() {
        IFile file = project.getFile("deletefile.xml");
        assertTrue(file.exists());
    ...
    public void testBranchText() {
        ...
        fileIntegrate.setTarget(toFile.getLocation().makeAbsolute()
                .toOSString());
        integrate.integrate(connection, fileIntegrate, 0, P4TeamUtils.createDefaultIntegration(connection), false);
    ...
    public void testIntegrateText() {
        ...
        integrate.integrate(connection, fileIntegrate, 0, options, false);
        IP4Resource resource = P4Workspace.getWorkspace().getResource(toFile);
        assertNotNull(resource);
        assertTrue(resource instanceof IP4File);
        IP4File p4File = (IP4File) resource;
