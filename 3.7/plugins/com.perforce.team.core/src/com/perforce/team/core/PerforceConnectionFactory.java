...
public final class PerforceConnectionFactory {
	 ...
	 * DIR_WILDCARD_SUFFIX
	 */
	protected static final String DIR_WILDCARD_SUFFIX = File.separatorChar + "*"; //$NON-NLS-1$
	...
	public static String formatFilename(String path) {
		...
		/*
		 * Only convert a path that: i) Contain a file separator char ii) Are not in
		 * depot syntax iii) Do not end in "/*" iv) Contain one of the special chars
		 */
		if (path.contains("/") || path.contains("\\")) {
			if (!path.startsWith("//") && !path.endsWith(DIR_WILDCARD_SUFFIX) && hasSpecialChars(path)) { //$NON-NLS-1$
