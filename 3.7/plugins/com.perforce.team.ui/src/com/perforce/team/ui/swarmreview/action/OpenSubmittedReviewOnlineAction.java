...
public class OpenSubmittedReviewOnlineAction extends SubmittedReviewAction {

	public OpenSubmittedReviewOnlineAction() {
		super();
			...
			IP4Runnable runnable = new P4Runnable() {
				...
				public void run(IProgressMonitor monitor) {
					if (selectedItems.length > 0) {
						...
						showOpenedBrowser(pendingChangeList, selectedItems, iserver);
					} else {
	...
	private void showOpenedBrowser(final IP4SubmittedChangelist pendingChangeList, final IP4Resource[] selectedItems,
			final IServer iserver) {
		PerforceUIPlugin.syncExec(new Runnable() {

			public void run() {
				initSwarmController(pendingChangeList,true);
				Result<Boolean> result = controller.extractReviewResultForChangeList(pendingChangeList.getId());
				if (!result.isError() && result.isSuccessful()) {
					try {
						controller.openSwarmReviewOnline();
					} catch (Exception e) {
						MessageDialog.openError(null, "Open Swarm", "Unable to open");
					}
				}
				if (result.isError()) {
					MessageDialog.openError(null, "Open Swarm", "Unable to open " + result.getMessage());
				}
	...
	public boolean isEnabledEx() {
		...
		if (pendingChangeList != null) {
			...
			Result<Boolean> result = controller.isOpenReview(pendingChangeList);
